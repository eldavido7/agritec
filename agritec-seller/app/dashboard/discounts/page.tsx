...
import {
  useSellerDiscountsStore,
  type SellerDiscountRecord,
} from "@/stores/seller-discounts-store";
...
const getDiscountState = (discount: SellerDiscountRecord) => {
...
const productLabel = (discount: SellerDiscountRecord, products: ReturnType<typeof useSellerProductsStore.getState>["products"]) => {
  ...
  const variantNames = discount.variantIds
    .map((variantId) => {
      const product = products.find((item) =>
        item.variants?.some((variant) => String(variant.id) === variantId),
      );
...
export default function DiscountsPage() {
  ...
  const { products, fetchProducts } = useSellerProductsStore();
  ...
  const [selectedDiscount, setSelectedDiscount] = useState<SellerDiscountRecord | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);

  useEffect(() => {
    if (!authReady) return;
    void fetchProducts();
    void fetchDiscounts({ includeInactive: true });
  }, [authReady, fetchDiscounts, fetchProducts]);
  ...
  const filteredDiscounts = useMemo(() => {
    return discounts.filter((discount) => {
      ...
      const searchable = [
        ...
        productLabel(discount, products),
      ]
        .join(" ")
        .toLowerCase();
      return (
  ...
  }, [discounts, searchQuery, products, stateFilter, typeFilter]);
  ...
  const openEdit = (discount: SellerDiscountRecord) => {
    setFormMode("edit");
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          <p className="text-muted-foreground">
            Manage discount codes for {sellerProfile?.farmName || "your farm"}. Products and variants
      ...
      >
        <div className="grid gap-3 lg:grid-cols-[1fr_auto_auto]">
          ...
          <div className="flex flex-wrap gap-2">
            {["All", "Active", "Scheduled", "Expired", "Inactive"].map((state) => (
              <Button
                key={state}
                    ...
                    {paginatedDiscounts.length > 0 ? (
                      paginatedDiscounts.map((discount) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="max-w-55 px-4 py-4 text-sm text-foreground">
                              {productLabel(discount, products)}
                            </td>
                      ...
                      <tr>
                        <td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">
                          {error || "No discounts match your filters."}
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      <DiscountFormModal
        ...
        onSubmit={async (discount) => {
          try {
            if (formMode === "edit" && selectedDiscount) {
              await updateDiscount(selectedDiscount.id, discount);
            ...
            toast.error(saveError instanceof Error ? saveError.message : "Unable to save discount");
          }
        }}
      />
      ...
      <DeleteDiscountModal
        ...
        onDelete={async (id) => {
          try {
            ...
            toast.error(deleteError instanceof Error ? deleteError.message : "Unable to delete discount");
          }
        }}
      />
    </div>
