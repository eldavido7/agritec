    ...
    for (const [groupIndex, group] of quote.sellerGroups.entries()) {
      ...
      await tx.sellerOrderGroup.create({
        data: {
          ...
          weightUnitSizeKg: group.weightUnitSizeKg ?? 0,
          shippingUnits: group.shippingUnits ?? 0,
          minimumFee: group.minimumFee ?? undefined,
          additionalUnitFee: group.additionalUnitFee ?? undefined,
